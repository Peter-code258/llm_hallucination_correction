...
class ClaimExtractor:
    ...
    def validate_claims(self, claims: List[Dict[str, Any]]) -> Dict[str, Any]:
        """验证提取的声明质量"""
        total_claims = len(claims)
        atomic_claims = sum(1 for claim in claims if claim.get('atomic', False))
        avg_confidence = sum(claim.get('confidence', 0) for claim in claims) / total_claims if total_claims > 0 else 0
        
        return {
            "total_claims": total_claims,
            "atomic_claims": atomic_claims,
            "non_atomic_claims": total_claims - atomic_claims,
            "avg_confidence": avg_confidence,
            "quality_score": (atomic_claims / total_claims * avg_confidence) if total_claims > 0 else 0
        }